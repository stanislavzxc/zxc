    ...
    async discount_getall () {
      ...
      try {
        ...
        for (let i = 0; i < this.discounts.length; i++) {
          this.discounts[i].expiration_date = this.discounts[i].expiration_date.slice(0, 10); 
        }
      } catch (error) {
        console.error("Error updating user:", error);
      }
...
<template>
  ...
  <div class="wrap-group">
    <div class="group">
      <label for="price_energy" class="group-value">Цена электроэнергии за KW/час</label>
      <input
        type="text"
        id="price_energy"
        name="price_energy"
...
<div class="card settings-price" v-else>
  ...
  <div class="par" v-for="discount in discounts" :key="discount.id">
    <div class="wrap-group">
      <div class="group">
        <label for="category" class="group-value">Товар</label>
        <select class="group-item" name="category" id="category" v-model="discount.miner_id">
          <option v-for="miner in miners" :key="miner.id" :value="miner.id">
            {{ miner.name }}
          </option>
        </select>
